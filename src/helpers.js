...
function getNodeTemplate(data,isExport){
    ...
    // return '<div class="center" style="width: 150px; height: 220px; background-color: green; "><img width="50" height="50" class="emp-image" id="img-'+data.empNumber+'" src="'+(isExport? data.image: '' )+'"/><h4>'+data.name+'</h4></div>';
}
...
window.onload = function() {
    ...
    fetch("https://9af6-2402-d000-8104-63f1-47d6-a3b0-b15b-9f71.in.ngrok.io/api/calendarFeed?feedUrl=koko",{
        method: "GET"})
        .then(response => response.json())
        .then(data => {
            console.log(data)
            updateGraphData(graph, data.data)
        });
...
function updateGraphData(data,imageData){
    ...
    var dataURL = 'data:image/png;base64,'+imageData.find((image)=>{return image.id == data.empNumber}).image
    // var dataURL = getImageURL(data.empNumber);
    img.src = dataURL;// getImageURL(data.empNumber);
